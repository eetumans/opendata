...
class AppFeedBlock extends BlockBase {
  ...
  public function build() {
    ...
    $recentApplicationsResponse = $client->request(
      'GET', 'http://localhost:8080/data/api/action/' .
      'package_search?fq=dataset_type:showcase' .
      '&sort=metadata_modified%20desc&rows=3'
    );
    $recentApplicationsResult = Json::decode(
      $recentApplicationsResponse->getBody()
    );
